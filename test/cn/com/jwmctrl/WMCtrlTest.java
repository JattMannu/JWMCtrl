...
public class WMCtrlTest {
		...
		if (Platform.isMac()) {
			...
			if (StringUtils.isBlank(System.getProperty("jna.library.path"))) {
				System.setProperty("jna.library.path",
						WMCtrl.MAC_DEFAULT_JNA_LIBRARY_PATH);
			}
		}
	}
	...
	@Test
	public void list_current_desktop() {
		Assert.assertTrue(WMCtrl.list_desktops(disp));
		String out = getOut();
		System.out.println("out = " + out);
		int currentDesktop = -1;
		try {
			currentDesktop = Integer.parseInt(out);
		} catch (Exception e) {
		}
		Assert.assertTrue(currentDesktop >= 0);
	}
	...
	@Test
	public void list_windows_in_stacking_order() {
		Assert.assertTrue(WMCtrl.list_windows(disp));
		String out = getOut();

		Options options = new Options();
		options.stacking_order = true;
		Assert.assertTrue(WMCtrl.list_windows(disp, options));
		String out2 = getOut();
		Assert.assertFalse(out.equals(out2));
	}
	...
	@Test
	public void iconify_window() {
		Assert.assertTrue(WMCtrl.iconify_window(disp,
				WMCtrl.get_active_window(disp)));
	}
